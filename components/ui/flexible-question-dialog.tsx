...
export default function FlexibleQuestionDialog({
  canvasRef,
  workflow,
}: IDialogFinalAnswerProps) {
  ...
  const isSubmitDisabled = () => {
    if (questionOptionsData && questionOptionsData.type === "multiple-choice") {
      ...
      return (
        questionOptionsData?.essayInputs?.some(
          (input) => !essayAnswers[input.id],
        ) || isLoading
      );
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="default" disabled={!workflow?.options}>
          Submit Answer
        </Button>
      </DialogTrigger>
            ...
            <DialogDescription className="pb-3">
              {questionOptionsData
                ? questionOptionsData.type === "multiple-choice"
                  ...
                  : "Provide your answers in the fields below."
                ...
                <div className="grid grid-cols-2 gap-4">
                  {questionOptionsData.options?.map((option) => (
                    <Button
                      ...
                      className={`w-full ${
                        answerSubmitted &&
                        option.id === questionOptionsData.correctAnswer
                    ...
                    <div key={input.id}>
                      ...
                      {input.type === "short" ? (
                        <Input
                          ...
                          onChange={(e) =>
                            handleEssayChange(input.id, e.target.value)
                          }
